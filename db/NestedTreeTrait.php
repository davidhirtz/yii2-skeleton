	...
	public function updateTreeBeforeSave()
	{
		...
		else
		{
			if($this->isAttributeChanged('parent_id', false))
			{
				...
				$branchIds=ArrayHelper::simplify($query->all(), false, 'id');
				...
				if($this->parent)
				{
					...
					$query=(new Query())->where(['not in', 'id', $branchIds])->andWhere(new Expression(':rgt BETWEEN [[lft]] AND [[rgt]]', ['rgt'=>$this->parent->getAttribute('rgt')]));
					static::updateAllCounters(['rgt'=>$diff], $query->where);

					// Update new right hand side of tree.
					$query=(new Query())->where(['not in', 'id', $branchIds])->andWhere(new Expression('[[lft]]>:rgt', ['rgt'=>$this->parent->getAttribute('rgt')]));
