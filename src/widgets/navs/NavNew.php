...
class NavNew extends Widget
{
    ...
    {
        return $this->items && (count($this->items > 1) || !$this->hideOneItem)
            ? Ul::make()
    ...
    {
        if (!$this->activateItems || $this->hasActiveItem) {
            return false;
        }
            ...
            if (is_array($item['active'])) {
                ...
                foreach ($routes as $route => $params) {
                    ...
                    if (preg_match("~$route~", (string)Yii::$app->controller->route)) {
                        ...
                        return $this->hasActiveItem = true;
                    }
                }
            }
        ...
        if (isset($item['url'][0]) && is_array($item['url'])) {
            return $this->hasActiveItem = $this->compareRoute($item['url']);
        }
