...
trait ActiveFormTrait
{
            ...
            foreach ($this->fields as $field) {
                ...
                if (!is_string($field) && is_callable($field)) {
                    ...
                    if ($field instanceof \yii\widgets\ActiveField) {
                        if (!$this->isSafeAttribute($field->attribute)) {
                            $methodName = (new ReflectionFunction($callback))->getName();
                            Yii::warning("Skipping unsafe attribute '$field->attribute' set by '$methodName'.");
                            continue;
                        }
                ...
                if (is_array($field)) {
                    $isVisible = is_array($field[0] ?? null)
                        ? ArrayHelper::remove($field[0], 'visible', true)
                        : ArrayHelper::remove($field, 'visible', true);
                ...
                if (!$callable) {
                    $options = [...$field ?? [], 'attribute' => $attribute];
                    $callable = $this->getAutogeneratedField(...);
                }

                if (!in_array($attribute, $this->i18nAttributes)) {
                    echo $callable($options);
                    continue;
                }

                $this->renderI18nCallback($callable, $options);
            }
        }
    }
    ...
    protected function getAutogeneratedField(array $options = []): ActiveField|string
    {
        ...
        if ($type == 'hidden') {
            ...
            return $field->hiddenInput($options)->parts['{input}'];
        }
    ...
    protected function renderI18nCallback(callable $callable, ?array $options = []): void
    {
