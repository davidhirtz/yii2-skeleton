    ...
    public function renderFields(): void
    {
        if ($this->fields) {
            foreach ($this->fields as $options) {
                ...
                $options['attribute'] ??= array_shift($options);
                ...
                if (!$this->isSafeAttribute($options['attribute'])) {
                    Yii::warning("Skipping unsafe attribute '{$options['attribute']}'.");
                    continue;
                }
                ...
                $methodName = lcfirst(Inflector::camelize($options['attribute'])) . 'Field';

                if ($this->hasMethod($methodName)) {
                    Yii::debug("Rendering '{$options['attribute']}' field with method '$methodName'");
                    $callable = $this->$methodName(...);
                } else {
                    $callable = $this->getAutogeneratedField(...);
                }

                echo in_array($options['attribute'], $this->i18nAttributes)
