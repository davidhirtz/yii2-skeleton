 ...
 * @see TimezoneDropDownList::getTimezones()
 */
class TimezoneDropDownList extends InputWidget
{
	/**
	 * @var array
	 */
	...
	public function run()
	{
		...
		{
			echo Html::dropDownList($this->name, $this->value, $timezones, $this->options);
		}
	}
	...
	public function getTimezones()
	{
		if($this->_timezones===null)
		{
			...
			/**
			 * Create a multi dimensional array to sort by offset first
			 * and display name after.
			 */
	...
	private function formatTimezoneName($name)
	{
		switch($name)
		{
			case 'America/Anchorage': return 'Alaska';
			case 'America/Los_Angeles': return 'Pacific Time (PST)';
			case 'America/Denver': return 'Mountain Time (MST)';
			case 'America/Dawson_Creek': return 'Arizona';
			case 'America/Chicago': return 'Central Time (CST)';
			case 'America/New_York': return 'Eastern Time (EST)';
			case 'America/Campo_Grande': return 'Brazil';
		}

		return strtr(preg_replace('#^(America|Arctic|Asia|Atlantic|Australia|Europe|Indian)/#', '', $name), array('_'=>' ', '/'=>' | '));
	}
}