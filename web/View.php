...
class View extends \yii\web\View
{
	...
	public function getPageTitle()
	{
		...
		return $title ? ($this->pageTitleTemplate ? strtr($this->pageTitleTemplate, ['{title}'=>$title, '{app}'=>Yii::$app->name]) : $title) : Yii::$app->name;
	}
	...
	public function registerTwitterCardMetaTags($card='summary_large_image', $title=null, $description=null)
	{
		...
		$this->registerMetaTag(['name'=>'twitter:title', 'content'=>$title ?: $this->getPageTitle()], 'twitter:title');
		$this->registerMetaTag(['name'=>'twitter:description', 'content'=>$description ?: $this->getDescription()], 'twitter:description');

		if(!empty(Yii::$app->params['twitter.siteName']))
		{
			$this->registerMetaTag(['name'=>'twitter:site', 'content'=>Yii::$app->params['twitter.siteName']], 'twitter:site');
		}
	}
