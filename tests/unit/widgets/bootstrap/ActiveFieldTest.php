...
class ActiveFieldTest extends Unit
{
    public function testHexColorField(): void
    {
        ...
        $expected = '<input type="color" id="test-color-color" class="input" name="Test[color]" value="#000000" required>';

        self::assertStringContainsString($expected, $field->hexColor()->render());

        $model->color = 'cccccc';
        $expected = '<input type="color" id="test-color-color" class="input" name="Test[color]" value="#cccccc" required>';
