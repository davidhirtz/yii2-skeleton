...
class GridView extends \yii\grid\GridView
{
	 ...
	 * @var array
	 */
	//public $headerRowOptions=['class'=>'text-nowrap'];
	...
	public function init()
	{
		...
		if(is_string($this->columns))
		{
			...
			{
				$column='render'.trim(Inflector::id2camel($column, '_')).'Column';
				$this->columns[]=call_user_func([$this, $column]);
			}
		}
		...
		if(!$this->rowOptions)
		{
			$this->rowOptions=function($record)
			{
				...
				{
					return ['id'=>$this->getFormName().'-'.implode('-', (array)$record->getPrimaryKey())];
				}
			};
		}
	...
	public function renderSummary()
	{
		...
		if($this->search)
		{
			...
			$summary.=Html::a(Html::tag('span', '&times;', ['aria-hidden'=>true]), $this->searchUrl, ['class'=>'close', 'aria-label'=>Yii::t('app', 'Close')]);
		}
	...
	public function renderHeader()
	{
		if($this->header)
		{
			return Html::tag('div', is_array($this->header) ? $this->renderRows($this->header) : $this->header, ['class'=>'grid-view-header']);
		}
	}
	...
	public function renderFooter()
	{
		if($this->footer)
		{
			return Html::tag('div', is_array($this->footer) ? $this->renderRows($this->footer) : $this->footer, ['class'=>'grid-view-footer']);
		}
	}
	...
	public function renderSearchInput()
	{
		...
		$options=[
			'class'=>'form-control',
			'prepend'=>Html::submitButton(FA::icon(ArrayHelper::remove($this->searchFormOptions, 'icon', 'search'), ['class'=>'fa-fw']), ['class'=>'btn-transparent']),
